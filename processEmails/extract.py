...
class EmailProcessor:
    ...
    def predict_label(self, email_body):
        prediction = self.model.predict([email_body])[0]
        return prediction
    ...
    def determine_status(self, body, subject):
        applied_phrases = [
        ...
        if self.predict_label(body) == "Applied" or self.keyword_search(body, applied_phrases) or self.keyword_search(subject, applied_phrases) :
            return "Applied"
        elif  self.predict_label(body) == "Rejected" or self.keyword_search(body, reject_phrases) or self.keyword_search(subject, reject_phrases) :
            return "Rejected"
        return "Irrelevant"
...
class ApplicationTracker:
    ...
    def update_application_tracker(self, emails_data, email_processor):
        ...
        for email_data in emails_data:
            status = email_processor.determine_status(email_data['Body'], email_data['Subject'])
